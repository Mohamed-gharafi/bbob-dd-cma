"""Original source taken from https://gist.github.com/youheiakimoto/1180b67b5a0b1265c204cba991fa8518
"""
...
class DdCma:
    ...
    def onestep(self, func):
        ...
        if not np.all(arf[idx[1:]] - arf[idx[:-1]] > 0.):
            warnings.warn("assumed no tie, but there exists")
...
if __name__ == "__main__":
    ...
    for restart in range(NUM_RESTART):        
        ...
        if total_neval < MAX_NEVAL and fbest > F_TARGET:
            ...
            break
'''
 # define a function to wrap the main loop
def fmin(func, x0, sigma0,
        MAX_NEVAL=np.inf,
        F_TARGET=-np.inf,
        flg_variance_update=True,
        NUM_RESTART=1,
    ...
    N = func.dimension
    total_neval = 0
    ddcma = DdCma(x0(), sigma0=np.ones(N)*sigma0, flg_variance_update=flg_variance_update)
    checker = Checker(ddcma)

    if log_data: logger = Logger(ddcma)
    
    # reshape the evaluation function
    def func_to_eval(f):
        def wrapper(x):
            return np.array([f(lx) for lx in x])
        return wrapper
    ...
    for restart in range(NUM_RESTART):
        ...
        while not issatisfied and ddcma.neval < MAX_NEVAL:
            ddcma.onestep(func=func_to_eval(func))
            fbest = np.min(ddcma.arf)
            fbestsofar = min(fbest, fbestsofar)
            if func.final_target_hit:
                ...
                issatisfied, condition = checker()
            if ddcma.t % 10 == 0:
                if verbose == True : print(ddcma.t, ddcma.neval, fbest, fbestsofar)
                if log_data == True : logger()
        if verbose == True : print("Terminated with condition: " + condition)
        if log_data == True : logger(condition)
        if condition == '':
            stopping_arr.append('maxeval')
        else:
            stopping_arr.append(condition)
        # For restart
        total_neval += ddcma.neval
        if total_neval < MAX_NEVAL and not func.final_target_hit:
            popsize = ddcma.lam * 2
            ddcma = DdCma(x0(), sigma0=np.ones(N)*sigma0, lam=popsize)
            checker = Checker(ddcma)
            if log_data == True : logger.setcma(ddcma)
            if verbose == True : print("Restart with popsize: " + str(ddcma.lam))
        else:
            return stopping_arr
    # Produce a figure
    if 11 > 3:
