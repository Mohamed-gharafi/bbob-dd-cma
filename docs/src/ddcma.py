...
def fmin(func, x0, sigma0,
    ...
    N = x0.shape[0]
    total_neval = 0
    ddcma = DdCma(xmean0=np.random.randn(N), sigma0=np.ones(N)*sigma0, flg_variance_update=flg_variance_update)
    checker = Checker(ddcma)
    ...
    for restart in range(NUM_RESTART):
        ...
        # For restart
        total_neval += ddcma.neval
        if total_neval < MAX_NEVAL and not F_TARGET:
            popsize = ddcma.lam * 2
            ddcma = DdCma(func.initial_solution_proposal(), sigma0=np.ones(N)*sigma0, lam=popsize)
            checker = Checker(ddcma)
