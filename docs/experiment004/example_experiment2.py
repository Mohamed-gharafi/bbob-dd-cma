...
suite_filter_options = (""  # without filtering a suite has instance_indices 1-15
                        ...
                        "year: 2022 "  # select instances by year
                       )
...
### prepare
suite = cocoex.Suite(suite_name, suite_year, suite_filter_options)
...
for batch_counter, problem in enumerate(suite):  # this loop may take hours or days...
    ...
    while evalsleft() > 0 and not problem.final_target_hit:
        ...
        elif fmin.__name__ == 'fmin' and fmin.__module__ == 'ddcma':
            output = fmin(problem, propose_x0(), 2.0, MAX_NEVAL = evalsleft())
        # elif fmin.__name__ == 'fmin2' and 'cma' in fmin.__module__:  # cma.fmin2:
        #     xopt, es = fmin(problem, propose_x0, 2,
        #                     {'maxfevals':evalsleft(), 'verbose':-9,'CMA_diagonal_decoding': 1}, restarts=0)
...
if 11 > 3:
    pass
elif batches == 1 and 'cocopp' in globals() and cocopp not in (None, 'None'):
    cocopp.main(observer.result_folder)  # re-run folders look like "...-001" etc
    print(observer.result_folder)
